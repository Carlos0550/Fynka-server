...
app.post("/create-administrator", upload.none(), async (req, res) => {
    ...
    const insertQuery = `
        INSERT INTO credenciales(email, nombre_usuario, contrasena, administrador, empleado) VALUES($1,$2,$3,$4,$5) RETURNING *
    `
    const insertQuery1 = `
    ...
    try {
        ...
        const result = await client.query(findAdminQuery, [username, email])
        ...
        console.log(email, username, "hashedPassword", true, false)
        ...
        console.log("ADMINISTRADOR ID: ", administrador_id)
...
app.post("/login-user", upload.none(), async (req, res) => {
    ...
    const getQuery1 = `
        SELECT id, contrasena, empleado, administrador FROM credenciales WHERE nombre_usuario = $1 OR email = $2;
    `

    const updateQuery = `
        UPDATE credenciales SET fecha_inicio_sesion = $1, fecha_reestablecer_autenticacion = $2, autenticado = $3 WHERE id = $4 RETURNING *
    `
    const client = await clientDb.connect()

    try {
        await client.query("BEGIN")
        const response1 = await client.query(getQuery1, [username, email])
